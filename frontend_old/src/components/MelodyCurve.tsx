  ...
  useEffect(() => {
    ...
    if (beats.length > 0 && duration > 0) {
      ...
      for (const beat of beats) {
        const x = padding + (beat / duration) * plotWidth;
        if (x >= padding && x <= width - padding) {
          ctx.fillRect(x - 1, height - padding + 2, 2, 6);
        }
      }
    }
    ...
    if (currentPitch) {
      ...
      ctx.font = "12px monospace";
      ctx.textAlign = "left";
      const labelX = Math.min(playheadX + 8, width - padding - 80);
      const labelY = Math.max(dotY - 8, padding + 12);
      ctx.fillText(`${currentPitch.frequency.toFixed(1)} Hz`, labelX, labelY);
    }
